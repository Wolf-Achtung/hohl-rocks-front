...
async function loadBubbles(){
  try{ const r=await fetch('/bubbles.json',{cache:'no-store'}); const list=r.ok?await r.json():[];
    const grid=$('#bubble-grid'); grid.innerHTML=list.map(b=>`
...
function openBubble(def){
  ...
  openModal(`<h2>${esc(def.title)}</h2>
    <form id="bubble-form">${formHtml(fields)}
      <div class="form-row" style="justify-content:flex-end"><button class="ui btn" type="submit">Generieren</button></div>
    </form><div class="result" id="bubble-result"></div>`);
  const form=$('#bubble-form');
  form.addEventListener('submit', async (ev)=>{
    ev.preventDefault(); const out={}; const fd=new FormData(form); const files={};
    ...
    try{ const payload={input:out, files}; const res=await apiJson('POST', `/api/bubble/${encodeURIComponent(def.id)}`, payload);
      renderResult(def,res); localStorage.setItem('bubble_'+def.id+'_lastOutput', JSON.stringify(res));
    }catch(e){ $('#bubble-result').innerHTML=`<p>Fehler: ${esc(String(e.message||e))}</p>`; }
  });
}
function renderResult(def,res){
  const box=$('#bubble-result'); const type=res.type||def.output||'text';
  if(type==='image'){ const url=res.url||res.data||''; box.innerHTML=url?`<img src="${esc(url)}" alt="KI‑Bild">`:'<p>Kein Bild erhalten.</p>'; }
  else if(type==='audio'){ const url=res.url||res.data||''; box.innerHTML=url?`<audio controls src="${esc(url)}"></audio>`:'<p>Kein Audio erhalten.</p>'; }
  else if(type==='html'){ box.innerHTML=res.html||res.data||'<p>Kein HTML‑Ergebnis.</p>'; }
  else { const text=res.text||res.data||''; box.innerHTML=text?`<pre style="white-space:pre-wrap">${esc(text)}</pre>`:'<p>Kein Text erhalten.</p>'; }
}
